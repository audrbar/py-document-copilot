...
async def load_messages(client: AsyncClient, thread_id: uuid.UUID) -> list[UIMessage]:
    ...
    assistant_ids = [
        message.id for message in messages if message.role == "assistant" and message.id
    ]
...
async def append_grounded_turn(
    ...
    *,
    ...
    await (
        client.table("chat_threads").update(updates).eq("id", str(thread_id)).execute()
    )
